...
#ifdef ESP32
//can use a different approach for ESP32
#include <EEPROM.h>
...
void dccPacketEngine(void) {
	...
	*/
	...
	portEXIT_CRITICAL_ISR(&timerMux);
		...
		//2021-12-23 note in ISR	
		//in principle, because it takes 8mS to transmit a packet, we have enough time in the rest of this routine
		//to write DCC packet values without causing a conflict with the ISR that also reads that structure.
		//The caveat here is whether the struct itself causes an issue because we are regularly writing to its 
		//timer flags.
...
//these routines are associated with local hardware display, keypad and jogwheel
...
/*call using &target, will put given digitASCII at the digPos position
...
#pragma endregion
		...
		//Analoge measurement exponentially smooths readings into the bus_mA value.
			//ESP32 is 12bit default resolution, 11dB default attenuation
