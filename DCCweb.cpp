...
//https://lastminuteengineers.com/creating-esp32-web-server-arduino-ide/   this is the single client webserver
//https://github.com/espressif/arduino-esp32/blob/master/libraries/WebServer/examples/FSBrowser/FSBrowser.ino
//looks a lot like my original ESP8266 code...

#ifdef ESP32
#include <WiFi.h>
#include <AsyncTCP.h>
#include <SPIFFS.h>
#include <FS.h>   //needed for file operations
#elif defined(ESP8266)
#error "ESP8266 not supported"
#endif

#include <ESPAsyncWebServer.h>
/*2022-1-11 apparently ESPAsyncWebserver will randomly crash and there's no fix for this at present.
even the standard webserver that's part of Wifi and is single client doesn't work that well, but I will give it a go
*/
...
void nsDCCweb::startWebServices() { 
	...
	//For ESP32, set the IP before the SSID
	//IPAddress class requires the address to be provided as 4 octets
	uint16_t myIP[4];  //esp32 needs uint16_t
	char *p = nullptr;
	char ipBoot[17];
	...
	//wait for the softAP to start, then confirm the ip address
	...
	web.serveStatic("/", SPIFFS, "/");  //default page is index.htm
	...
	web.onNotFound([](AsyncWebServerRequest *request) {request->send(404, "text/plain", "404: Not found");});
...
//the web server does not need to be called in the loop
