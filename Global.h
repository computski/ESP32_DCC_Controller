...
#define WEMOSR32_AND_L298_SHIELD
...
#elif defined(WEMOSR32_AND_L298_SHIELD)

//need to look at the pins, also uint32_t is required

//ESP32 is 12bit default resolution, 11dB default attenuation. FSD is 2.6v.  The L298 shield uses a 1.65 ohm shunt
//so max currrent we can read is 1570mA for FSD.  Single chanenl scaling is 2.59, parallel channels is 5.19
#define USE_ANALOG_MEASUREMENT
//#define ANALOG_SCALING 2.59
#define ANALOG_SCALING 5.19  //when using A and B in parallel
//Important: The Wemos R32 breaks out A12 and A10 in positions 0/1 these are IO 2 and 4 and unfortunately these are
//ADC2 and are disabled if Wifi is active.  We need to jumper A0 to A4 and A1 to A5 on the L298 shield, so that the
//voltages are presented to the ADC1 pins that sit behind A1 and A5.


//these are IO pin numbers

#define	PIN_HEARTBEAT 17  //jogwheel pushbutton Otherwise use 17
//On the WeMos R32, the built-in LED is on IO2, which is an AD input


#define DCC_PINS \
dcc_init(19,25,false,false);\
dcc_init(18,23,false,false);

//uint32_t dcc_info[4] = { PERIPHS_IO_MUX_MTDI_U,  19, 19 , 0 }; \
//uint32_t enable_info[4] = { PERIPHS_IO_MUX_MTDI_U,  25, 25 , 0 }; \
//uint32_t dcc_infoA[4] = { PERIPHS_IO_MUX_MTDI_U,  18, 18 , 0 }; \
//uint32_t enable_infoA[4] = { PERIPHS_IO_MUX_MTDI_U,  23,23 , 0 };
